defmodule SigncraftWeb.UserSettingsController do
  ...
  def update(conn, %{"action" => "update_username"} = params) do
    ...
    case Accounts.apply_user_username(user, password, user_params) do
      {:ok, applied_user} ->
        ...
        |> put_flash(
          :info,
          "A link to confirm your username change has been sent to the new address."
        )
  ...
  def confirm_username(conn, %{"token" => token}) do
    case Accounts.update_user_username(conn.assigns.current_user, token) do
      :ok ->
        conn
      ...
      :error ->
        conn
        |> put_flash(:error, "username change link is invalid or it has expired.")
        |> redirect(to: ~p"/users/settings")
    end
  end
